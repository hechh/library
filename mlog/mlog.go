package mlog

import (
	"strings"
	"sync"
	"time"
)

const (
	LOG_TRACE = 1
	LOG_DEBUG = 2
	LOG_WARN  = 3
	LOG_INFO  = 4
	LOG_ERROR = 5
	LOG_FATAL = 6
)

var (
	logObj   = NewLogger(LOG_DEBUG, &StdWriter{})
	dataPool = sync.Pool{New: func() any { return NewData() }}
)

func get(times int) IData {
	obj := dataPool.Get().(IData)
	obj.Add(int32(times))
	return obj
}

func put(obj IData) {
	if obj.Done() == 0 {
		dataPool.Put(obj)
	}
}

type Meta struct {
	Level int32
	Msg   string
	//FileName string
	//Line     int
	//FuncName string
}

type IData interface {
	Now() time.Time // 获取时间
	Add(int32)      // 并发次数
	Done() int32    // 完成次数
	Write(Meta)     // 写入数据
	Read() []byte   // 读取数据
}

type IWriter interface {
	Push(IData) // 推送日志
	Close()     // 关闭
}

func Init(mode int, level string, lpath string, lname string) {
	logObj.Close()
	switch mode {
	case 1: // 调试模式
		logObj = NewLogger(level, &StdWriter{})
	case 2: // 开发模式
		logObj = NewLogger(level, &StdWriter{}, NewLogWriter(lpath, lname))
	case 3: // 发布模式
		logObj = NewLogger(level, NewLogWriter(lpath, lname))
	default:
		logObj = NewLogger(level, &StdWriter{}, NewLogWriter(lpath, lname))
	}
}

func Close() {
	logObj.Close()
}

func Filter(names ...string) {
	logObj.Filter(names...)
}

func Tracef(format string, args ...any) {
	logObj.Trace(1, "", format, args...)
}

func Debugf(format string, args ...any) {
	logObj.Debug(1, "", format, args...)
}

func Warnf(format string, args ...any) {
	logObj.Warn(1, "", format, args...)
}

func Infof(format string, args ...any) {
	logObj.Info(1, "", format, args...)
}

func Errorf(format string, args ...any) {
	logObj.Error(1, "", format, args...)
}

func Fatalf(format string, args ...any) {
	logObj.Fatal(1, "", format, args...)
}

func Trace(skip int, attr, format string, args ...any) {
	logObj.Trace(skip+1, attr, format, args...)
}

func Debug(skip int, attr, format string, args ...any) {
	logObj.Debug(skip+1, attr, format, args...)
}

func Warn(skip int, attr, format string, args ...any) {
	logObj.Warn(skip+1, attr, format, args...)
}

func Info(skip int, attr, format string, args ...any) {
	logObj.Info(skip+1, attr, format, args...)
}

func Error(skip int, attr, format string, args ...any) {
	logObj.Error(skip+1, attr, format, args...)
}

func Fatal(skip int, attr, format string, args ...any) {
	logObj.Fatal(skip+1, attr, format, args...)
}

func LevelToString(level int32) string {
	switch level {
	case LOG_TRACE:
		return "TRACE"
	case LOG_DEBUG:
		return "DEBUG"
	case LOG_WARN:
		return "WARN"
	case LOG_INFO:
		return "INFO"
	case LOG_ERROR:
		return "ERROR"
	case LOG_FATAL:
		return "FATAL"
	}
	return ""
}

func StringToLevel(str string) int32 {
	switch strings.ToUpper(str) {
	case "TRACE":
		return LOG_TRACE
	case "DEBUG":
		return LOG_DEBUG
	case "WARN":
		return LOG_WARN
	case "INFO":
		return LOG_INFO
	case "ERROR":
		return LOG_ERROR
	case "FATAL":
		return LOG_FATAL
	}
	return LOG_WARN
}
